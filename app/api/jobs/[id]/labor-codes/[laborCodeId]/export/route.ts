 ...
 * Export time entries for a specific labor code to Excel
 */
export async function GET(
    ...
    try {
        ...
        // Fetch time entries for this job and labor code
        ...
        const worksheet = workbook.addWorksheet('Labor Hours')
        ...
        // Create filename
        const filename = `job_${job.jobNumber}_labor_${laborCode.code}.xlsx`
        ...
        console.error('Error exporting labor hours:', error)
        return NextResponse.json(
            {
                success: false,
                error: error.message || 'Failed to export labor hours'
            },
            { status: 500 }
        )
    }
