...
import { AssembliesListView } from '@/components/parts/assemblies-list-view'
...
export default async function AssembliesPage({ searchParams }: AssembliesPageProps) {
  ...
  try {
    ...
    const where: any = {}
    if (statusFilter && ['DRAFT', 'QUOTE', 'ASSEMBLY'].includes(statusFilter)) {
    ...
    return <AssembliesListView initialBOMs={bomsData} />
  } catch (error: any) {
    console.error('Error loading assemblies:', error)
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <h2 className="text-lg font-semibold text-red-900 mb-2">Error Loading Assemblies</h2>
